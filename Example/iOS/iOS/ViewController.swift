...
//    self.oldReload()
  ...
  func reloadData1() {
    ...
    URLRequest.requestFor(path: "https://jsonplaceholder.typicode.com/users")
      ...
      .then{ (users) -> [User] in
        self.reloadCount += 1
        if 0 == self.reloadCount % 5 {
          self.title = "Error simulation"
          throw ViewControllerError.simulatedError
        }
        self.title = "Pull \(5 - self.reloadCount % 5) more time to get error"
        return users
      }
      .onComplete { (_) in
        self.tableView.refreshControl?.endRefreshing()
      ...
      .onError { (error) in
        let alert = UIAlertController(title: "Network connection required!", message: error.localizedDescription, preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Ok", style: .cancel, handler: nil))
        self.present(alert, animated: true, completion: nil)
    }
  }
